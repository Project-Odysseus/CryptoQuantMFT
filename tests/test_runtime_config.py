...
def test_runtime_config_parses_strategy_settings() -> None:
    ...
    assert runtime_config.trading_symbol == "ETH/EUR"
...
def test_runtime_config_round_trips_to_disk(tmp_path: Path) -> None:
    ...
    runtime_config = RuntimeConfig(mode="paper", strategy_name="momentum_breakout", strategy_params={"lookback": 3}, trading_symbol="ETH/EUR", config_path=config_path, state_path=tmp_path / "runtime.state.json")
    ...
    assert reloaded.trading_symbol == "ETH/EUR"
...
def test_runtime_config_loads_existing_file_and_applies_cli_overrides(tmp_path: Path) -> None:
    ...
    assert runtime_config.trading_symbol is None
