...
- [ ] **Research ↔ runtime signal parity (found 2026-09-25 while building the research toolkit)**
  ...
  - [ ] **Re-entry after a forced exit.** After a risk stop (drawdown/time/ATR) closes a position, the next bar re-enters if the strategy still signals the same side. This holds in the backtester and in both runtime paths (`elif signal > 0.0 and position_size == 0.0`), so a stop only resets the entry price instead of keeping the position out. Needs a "don't re-enter until the signal resets" rule before stop-losses can be researched or relied on.
  - [ ] **Bar interval and warmup.** The robust candidates from `docs/research_log.md` run on 4h/1d bars and need 40-100 bars of history. The runtime builds bars from live ticks (`interval_seconds` default 1s) and has no historical warmup, so it would need a 4h/1d interval *and* seeding from `load_or_fetch_kraken_history` before any of them can be paper traded meaningfully.
