...
- [ ] **Position-level risk controls: closeout for the exchange-backed path**
  ...
  - [ ] `run_exchange_cycle()` does **not** yet get the time-stop / ATR stop-loss exits. Reason: both need "how long has this position been open" state, and nothing today persists an entry timestamp for a position reconstructed from the execution adapter/account tracker each cycle (`_current_account_state` only returns `avg_entry_price`, not when it was set). Before wiring this in, either (a) persist an entry timestamp in the runtime checkpoint or account-state tracker, or (b) derive it from the trade logger's most recent opening trade for the current position. Don't fake this with an approximation — get the persisted state right first.
